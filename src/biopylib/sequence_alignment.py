...
class PSA(SQ,aliView):
        ...
        # nicer print variant of result matrices
        def pprint(self,ids='SM'):
            
            if(ids is 'SM'):
                ldf = pd.DataFrame(self.SM)
            elif(ids is 'TM'):
                ldf = pd.DataFrame(self.TM)
            
            ldf.index = ['gap'] + [i for i in self.seq1]
            ldf.columns = ['gap'] + [i for i in  self.seq2]
            display(ldf)
